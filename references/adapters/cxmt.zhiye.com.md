...
7. **跨年代的日期不要硬刚**：`.phoenix-calendar-prev-year-btn` 连点可用（2026 → 2020 成功），
   但**年份面板格的点击不落值**（点 2019 后面板直接关掉、值不变）→ 2019 及更早的年份在本站控件上
   写不进去。本次 15 个项目里只有 3 个（2019 年的竞赛/室内定位/售货机）没填上，交用户手点即可，
   不要为了“全自动”反复重试。
8. **论文/获奖/实习区块是可重复添加的**：存在 `添加论文/专著`、`添加获奖情况`、`添加实习经历`、
   `添加项目经历` 按钮 → 别像我一开始以为“只能填一条”。
9. 项目条目重渲染后**引用会失效**：不要先拿到 entry 容器再连续写多个字段，
