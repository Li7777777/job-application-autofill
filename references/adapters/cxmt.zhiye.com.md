...
### 7.1 草稿会持久化，但**不是所有东西**
实测（刷新页面后回读）：
- ✅ **持久化**：文本/textarea/单选/大多数下拉（含项目 15 条名称+描述、获奖 6 条、论文、教育、高考分）——
  重新打开 `/form?fromPage=job&jobAdId=…&userId=…` 或直接刷新，内容还在；
  所以之前那个「0 请求 = 不自动保存」的结论是**过度悲观**的（真实情况是：值先进站点 state，再由站点自己同步；
  没进 state 的 DOM 写入会被这个同步覆盖掉——这正是“数据又没了”的机制）。
- ❌ **会丢**：**文件类字段**（证件照/简历附件/英语成绩截图/成绩单）和**刚刚改、还没同步的改动**（如刚选完两个下拉就刷新）。
  → 所以**提交前最后一步要重新确认这 3 个附件还在**，丢了就重传。

### 7.2 下拉（`.phoenix-select`）的两个操作坑
1. **选项渲染要等**：真实点击触发后，`.phoenix-selectList__listItem` 约 1.5s 才挂上；
   只等 0.9s 会得到 `no-element`（以为没弹开）。用 `seq ... 1600`。
2. **上一个下拉还开着时，再点触发会把它关掉**：
   如果你刚刚诊断过/点开过那个下拉，下一次 click 触发变成“关闭”，接着找选项就 `no-element`。
   → `seq` 的第一步固定加一个“先关掉已开弹层”的步骤：
   ```js
   (() => { document.body.click(); return document.body; })()
   ```

### 7.3 一个岗位填完、换投另一个岗位
- 直接开新岗位详情页 → 点「立即投递」→ 新的 `/form?...&jobAdId=新id...`；
- **已投递的岗位不影响新草稿**；新草稿会从账号简历/上一份草稿带到大部分内容（实测 192 字段里 134 已填）；
- 要重做的只有：两个地点类下拉 + 3~4 个附件 + 用户指定的差异项。
